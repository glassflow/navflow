...
class Store:
    ...
    def list_catalog_agents(self) -> list[dict]:
        with self._lock:
            rows = self.con.execute(
                ...
                "webhook_url, webhook_token, mcp_servers, updated_at, owned_by, customized "
        ...
        return [
            {"name": r[0], "trigger": r[1], "prompt": r[2], "slack_webhook": r[3] or "",
             ...
             "owned_by": r[10], "customized": bool(r[11])}
            for r in rows
        ]
    ...
    def list_mcp_servers(self) -> list[dict]:
        with self._lock:
            rows = self.con.execute(
                "SELECT name, url, auth_header, auth_value, updated_at, owned_by, customized "
                "FROM mcp_servers ORDER BY name").fetchall()
        return [{"name": r[0], "url": r[1], "auth_header": r[2] or "",
                 "auth_value": r[3] or "", "updated_at": r[4],
                 "owned_by": r[5], "customized": bool(r[6])} for r in rows]
