"""Remote MCP + NAVFLOW_AUTH_TOKEN — the MCP endpoint requires the bearer token from the agent, and
...
async def main():
    for p in (DB, DB + ".wal"):
        if os.path.exists(p):
            os.remove(p)
    base = {**os.environ, "NAVFLOW_OTLP_GRPC_PORT": "off", "NAVFLOW_AUTH_TOKEN": TOKEN}
    daemon = subprocess.Popen(
        [sys.executable, "-c", "from navflow.cli import run_daemon; run_daemon()"],
        env={**base, "NAVFLOW_DB": DB, "NAVFLOW_CATALOG": "/tmp/none_ma.yaml", "NAVFLOW_PORT": DPORT},
